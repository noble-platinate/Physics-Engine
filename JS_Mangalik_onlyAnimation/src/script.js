import * as THREE from "three";
        ...
        var substep = 3; // for fbd it goes from 1 to n_b + n_p , for contraint 1 to n_s
        ...
        const alldata = require("D:/Mangalik_Mitra/Engphile_Labs_Internship/JS_Project/src/to_be_checked/all_dick_"+alld_question_no + ".json");
        ...
        const geometryc = new THREE.ConeGeometry(0.15, 0.25, 32);
        ...
        for (i = 1; i < n_b + 1; i++) {
            ...
            for (j = 0; j < n_n; j++){
                ...
                //camera_shift = 0//-(meshk[i - 1].position.x - 1.5) *0.11
                    ...
                    var cone_angle
                    var normal_length = (blockdata[i].size[0])/17//length of normal proportional to magnitude
                    var normal_points = []
        ...
        */
            ...
            for (j = 0; j < linestr.length; j++) {
                ...
                if ((stringdata[j + 1]["first obj type"] == "<class 'blocksclass.pulleys'>" && (stringdata[j + 1]["first obj"] - 1) == i) ||
                    ...
                    if (alldata["bigblock dick"] == undefined || bigblockdata.mass == 0 || bigblockdata.type == "tp" || checker == 0) {
                        ...
                        var dis_from_p = Math.pow(1.6 * 1.6 - Math.pow(pulleydata[i + 1].radius / 15, 2), 0.5)
                        if (stringdata[j + 1]["second obj type"] == "<class 'blocksclass.pulleys'>" && (stringdata[j + 1]["second obj"] - 1) == i) {
        ...
        for (i = 1; i < n_p + 1; i++) {
            for (j = 0; j < ip_x[i + n_b - 1].length; j++) {
                ...
                /*if (alldata["bigblock dick"] == undefined || bigblockdata.mass == 0 || bigblockdata.type == "tp") {*/
